...
from amaranth                          import Cat, Const, ResetInserter, Signal, Elaboratable, Module, Mux
from amaranth.build                    import Attrs, DiffPairs, Resource
...
class USBAnalyzerSpeedDetector(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        with m.FSM(domain='usb', reset='INITIALIZE') as fsm:
            m.d.usb += self.state.eq(fsm.state)
...
class USBAnalyzerApplet(Elaboratable):
    ...
    def elaborate(self, platform):
        ...
        m.submodules.speed_detector = speed_detector = USBAnalyzerSpeedDetector()
        speed_selection = Mux(state.current[1:3] == 0b11, speed_detector.current_speed, state.current[1:3])
        ...
        pmod_b = platform.request("user_pmod", 1, dir="o")
        m.d.comb += [
            pmod_b[0].eq(speed_detector.state[0]),
            pmod_b[1].eq(speed_detector.state[1]),
            pmod_b[2].eq(speed_detector.state[2]),
            pmod_b[3].eq(speed_detector.state[3]),
            pmod_b[4].eq(utmi.line_state[0]),
            pmod_b[5].eq(utmi.line_state[1]),
            pmod_b[6].eq(utmi.xcvr_select[0]),
            pmod_b[7].eq(usb_diff),
        ]
